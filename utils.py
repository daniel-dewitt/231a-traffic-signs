...
def get_positive_features(train_path_pos, cell_size, window_size, block_size, nbins):
    ...
    image_files = [os.path.join(train_path_pos, f) for f in os.listdir(train_path_pos) if f.endswith('.png')]
    ...
    for i in range(num_images):
        img = imread(image_files[i],flatten=True)
...
def get_random_negative_features(non_face_scn_path, cell_size, window_size, block_size, nbins, num_samples):
    ...
    image_files = [os.path.join(non_face_scn_path, f) for f in os.listdir(non_face_scn_path) if f.endswith('.png')]
    num_images = len(image_files)
...
def compute_hog_features(im, pixels_in_cell, cells_in_block, nbins):
    ...
    for w in range(0, width - total_cells_in_block, int(stride)):
        for h in range(0, height - total_cells_in_block, int(stride)):
